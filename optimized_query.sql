...
select *
,RANK() over(partition by employee_id order by change_date desc) sal_rn_desc
,RANK() over(partition by employee_id order by change_date asc) sal_rn_asc
,LEAD(salary)over(partition by employee_id order by change_date) as next_sal
...
from salary_history)
-------------------------------------------------------------------------------------------------------------------------
--Here sal_always_increased --> (1:always incr , 0: for null case , -1: decreased once)
...
*/
-------------------------------------------------------------------------------------------------------------------------
select ls.employee_id
,max(case when sal_rn_desc=1 then salary end) as latest_salary
,sum(case when promotion='Yes' then 1 else 0 end) as total_promotions
,MAX(CAST((next_sal - salary)/salary*100 AS decimal(10,2))) max_hike
,MAX(CASE WHEN si.employee_id IS NOT NULL THEN 'YES' ELSE 'NO' END) AS sal_inc_or_not
,AVG(DATEDIFF(MONTH,change_date,next_date)) average_months_for_promotion
,rank()over(order by sg.salary_ratio desc,sg.join_date asc ) as rank_by_salary_ratio
from latest_salary_cte ls
left join salary_growth_cte sg on ls.employee_id=sg.employee_id
left join salary_increased si on si.employee_id=ls.employee_id 
group by ls.employee_id, sg.salary_ratio,sg.join_date
